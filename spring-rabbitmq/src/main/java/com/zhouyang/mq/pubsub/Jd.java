...
public class Jd {
    public static void main(String[] args) throws Exception {
        ...
        //声明队列
        channel.queueDeclare(RabbitConstant.QUEUE_JD, true, false, false, null);
        //队列绑定交换机 参数1：队列名称  参数2：交换机名称  参数3：路由key(暂时用不到)
        channel.queueBind(RabbitConstant.QUEUE_JD, RabbitConstant.EXCHANGE_WEATHER, "");
        channel.basicQos(1);
        //消费消息
        channel.basicConsume(RabbitConstant.QUEUE_JD, false, new DefaultConsumer(channel) {
            @Override
            public void handleDelivery(String consumerTag, com.rabbitmq.client.Envelope envelope, com.rabbitmq.client.AMQP.BasicProperties properties, byte[] body) throws java.io.IOException {
                System.out.println("京东收到气象信息：" + new String(body));
